...
We aimed to investigate the impact of labor-market experience on earnings by implementing an IV regression via 2SLS estimation. The labor-market experience (*experience*) was defined as a predictor variable. The salary (*wage*) was defined as a criterion variable. Within a standard OLS regression, this estimate may be problematic, since it can be argued that *experience* may suffer from endogeneity issues (i.e., it may be correlated with the regression error term). By employing a 2SLS estimation, we defined *age* as the natural exogenous instrument for *experience*. To fit this model, we can simply follow the indications represented below: 
...
The *Weak instruments test* examines the null hypothesis that the instrumental variable (i.e., *age*) is weakly correlated with the endogenous variable (i.e., *experience*). A significant p-value, as in this example, sustains the alternative hypothesis that the instrument is strong (see Stock & Watson, 2015), also ensuring the satisfaction of the instrument relevance condition.

*Wu-Hausman’s specification test* examines the null hypothesis that *experience* satisfies the exogeneity assumption.  A significant p-value, as in the example, provided evidence that *experience* suffers from endogeneity and, therefore, that the 2SLS approach is recommended over the standard OLS regression.
...
Let’s take a look at the *“Parameter Estimates”* table. We show that the unstandardized regression coefficient linking *education* to *wage* was 24.734, p<.001, whilst the standardized regression coefficient was 0.389, p<.001. It basically means that for each additional year of *labor-market experience*, *wage* is expected to increase by 24.734. With respect to the standardized coefficient, a change of 1 standard deviation in *experience* is associated with a change of 0.389 standard deviations in *wage*. 
